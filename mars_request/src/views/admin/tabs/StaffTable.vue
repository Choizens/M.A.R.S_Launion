<template>
  <div class="flex flex-col gap-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      ...
      <button @click="$emit('open-modal')" class="px-6 py-2.5 bg-[#004d66] text-white font-bold rounded hover:bg-[#003d52] transition-colors shadow-md flex items-center gap-2">
        <component :is="UserIcon" class="w-5 h-5" /> Add Staff
      </button>
    </div>
        ...
        <tbody class="divide-y divide-slate-200">
          ...
          <tr v-for="stf in staffList" :key="stf.id" class="hover:bg-slate-50 transition-colors" :class="{'opacity-60': !stf.is_active}">
            <td class="px-6 py-4 font-bold text-[#00334d]">
              <div class="flex flex-col">
                <div class="flex items-center gap-2">
                  ...
                  <span class="px-1.5 py-0.5 bg-blue-100 text-blue-700 text-[0.55rem] font-black uppercase rounded border border-blue-200">Staff</span>
                </div>
            ...
            <td class="px-6 py-4 text-center flex justify-center gap-2">
              <button @click="$emit('edit', stf)" class="p-2 text-cyan-600 hover:bg-cyan-50 rounded transition-colors" title="Edit Staff">
                <component :is="CogIcon" class="w-5 h-5" />
              </button>
              <button @click="$emit('delete', stf.id)" class="p-2 text-red-600 hover:bg-red-50 rounded transition-colors" title="Delete Staff">
