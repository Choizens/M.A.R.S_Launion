      ...
      <div class="bg-white w-full max-w-md rounded-lg shadow-2xl border-4 border-[#103059] flex flex-col overflow-hidden">
        ...
        <form @submit.prevent="$emit('submit')" class="p-6 flex flex-col gap-5">
          <div class="flex flex-col gap-1">
            ...
            <input v-model="form.date" type="date" required class="border px-4 py-2.5 rounded focus:border-[#103059] outline-none font-bold text-[#103059]" />
          </div>
          
          <div class="grid grid-cols-3 gap-4">
            <div class="flex flex-col gap-1">
              <label class="text-[0.65rem] font-black uppercase text-slate-500 tracking-wider">Morning Max</label>
              <input v-model.number="form.morning_slots" type="number" required class="border px-4 py-2.5 text-sm rounded focus:border-[#103059] outline-none font-bold text-[#103059]" />
            </div>
            <div class="flex flex-col gap-1">
              <label class="text-[0.65rem] font-black uppercase text-slate-500 tracking-wider">Afternoon Max</label>
              <input v-model.number="form.afternoon_slots" type="number" required class="border px-4 py-2.5 text-sm rounded focus:border-[#103059] outline-none font-bold text-[#103059]" />
            </div>
            <div class="flex flex-col gap-1">
          ...
          <div class="flex flex-col gap-1">
            ...
            <textarea v-model="form.reason" rows="2" placeholder="e.g. National Holiday, Staff Meeting..." class="border px-4 py-2.5 rounded focus:border-[#103059] outline-none text-sm"></textarea>
          </div>
