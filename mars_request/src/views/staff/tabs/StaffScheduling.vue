<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      ...
      <button
        @click="openSlotModal()"
        class="px-6 py-2.5 bg-[#ffca28] text-[#103059] font-black rounded border-2 border-[#103059] hover:bg-white transition-all shadow-md flex items-center gap-2"
      >
    ...
    <div v-else class="grid grid-cols-1 xl:grid-cols-3 gap-8">
      ...
      <div class="xl:col-span-2 bg-white rounded-lg shadow-sm border border-[#e2e8f0] p-6 lg:p-8">
        <div class="flex items-center justify-between mb-8">
          <h3 class="font-black text-[#103059] uppercase tracking-widest flex items-center gap-2">
          ...
          <div class="flex gap-2">
            <button @click="prevMonth" class="p-2 hover:bg-slate-100 rounded border transition-colors shadow-sm active:scale-95" title="Previous Month">
              <ChevronLeftIcon class="w-5 h-5 text-[#103059]" />
            </button>
            <button @click="nextMonth" class="p-2 hover:bg-slate-100 rounded border transition-colors shadow-sm active:scale-95" title="Next Month">
              <ChevronRightIcon class="w-5 h-5 text-[#103059]" />
            </button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-7 gap-2">
          ...
          <div
            ...
            :class="[
              ...
              date.isToday ? 'border-[#103059]' : 'border-slate-100',
          ...
          >
            <div class="flex justify-between items-start">
              <span v-if="date.day" :class="['text-xs font-black', date.isToday ? 'text-white bg-[#103059] px-1.5 py-0.5 rounded' : 'text-slate-400']">
                {{ date.day }}
              </span>
        ...
        <div class="mt-8 flex flex-wrap gap-6 border-t pt-6">
          ...
          <div class="flex items-center gap-2 text-[0.65rem] font-bold text-slate-500 uppercase">
            <div class="w-3 h-3 rounded bg-white border-2 border-[#103059]"></div> Today
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-[#e2e8f0] overflow-hidden flex flex-col">
        <div class="px-6 py-4 bg-slate-50 border-b">
          <h3 class="font-bold text-[#103059] text-sm uppercase tracking-wide">Slot Records</h3>
        </div>
              ...
              >
                <td class="px-6 py-4">
                  <div class="flex flex-col">
                    <span class="font-bold text-[#103059]">{{ formatDate(slot.date) }}</span>
        ...
        <div class="bg-white w-full max-w-md rounded-lg shadow-2xl border-4 border-[#103059] flex flex-col overflow-hidden">
          ...
          <form @submit.prevent="handleSlotSubmit" class="p-6 flex flex-col gap-5">
            <div class="flex flex-col gap-1">
              ...
              <input v-model="slotForm.date" type="date" required class="border px-4 py-2.5 rounded focus:border-[#103059] outline-none font-bold text-[#103059]" />
            </div>

            <div class="grid grid-cols-3 gap-4">
              <div class="flex flex-col gap-1">
                <label class="text-[0.65rem] font-black uppercase text-slate-500 tracking-wider">Morning Max</label>
                <input v-model.number="slotForm.morning_slots" type="number" min="0" required class="border px-4 py-2.5 text-sm rounded focus:border-[#103059] outline-none font-bold text-[#103059]" />
              </div>
              <div class="flex flex-col gap-1">
                <label class="text-[0.65rem] font-black uppercase text-slate-500 tracking-wider">Afternoon Max</label>
                <input v-model.number="slotForm.afternoon_slots" type="number" min="0" required class="border px-4 py-2.5 text-sm rounded focus:border-[#103059] outline-none font-bold text-[#103059]" />
              </div>
              <div class="flex flex-col gap-1">
            ...
            <div class="flex flex-col gap-1">
              ...
              <textarea v-model="slotForm.reason" rows="2" placeholder="e.g. National Holiday, Staff Meeting..." class="border px-4 py-2.5 rounded focus:border-[#103059] outline-none text-sm"></textarea>
            </div>

            <div v-if="editingSlotId" class="flex gap-3">
              <button type="submit" :disabled="submitting" class="flex-1 py-3.5 bg-[#ffca28] text-[#103059] font-black uppercase tracking-widest rounded border-2 border-[#103059] hover:bg-white transition-all shadow-md active:scale-95 disabled:opacity-50">
            ...
            <button v-else type="submit" :disabled="submitting" class="py-3.5 bg-[#ffca28] text-[#103059] font-black uppercase tracking-widest rounded border-2 border-[#103059] hover:bg-white transition-all shadow-md active:scale-95 disabled:opacity-50">
